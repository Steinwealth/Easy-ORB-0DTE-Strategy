...
class PrimeORBStrategyManager:
    ...
    def _get_volume_color(self, symbol: str, intraday_data: List[Dict[str, Any]]) -> str:
        """Get volume color from validation candle (7:00-7:15 AM PT). Not the opening range (that is 6:30-6:45)."""
        try:
    ...
    def _check_prev_candle_vs_orb(self, symbol: str, intraday_data: List[Dict[str, Any]], orb_level: float, above: bool) -> bool:
        """Check if validation candle (7:00-7:15 AM PT; NOT opening range) closed above/below ORB level (ORB = 6:30-6:45)."""
        try:
            prev_candle_start = time(7, 0)
            prev_candle_end = time(7, 15)
        ...
        try:
            ...
            if symbol in ['SPY', 'QQQ', 'SPX'] and intraday_data:
                ...
                if first_bar and 'timestamp' in first_bar:
                    ...
                    if isinstance(bar_ts, datetime):
                        ...
                        log.debug(f"🔍 {symbol} SO Check: intraday_data has {len(intraday_data)} bars, first bar timestamp={bar_ts.strftime('%H:%M:%S %Z')}, bar_time={bar_time}, window_match={time(7, 0) <= bar_time <= time(7, 15)}")
            ...
            if distance_from_high >= self.so_bullish_threshold:
                ...
                if validation_close_715 is not None:
                    ...
                    log.info(f"   Previous candle check (7:15 close {validation_close_715:.2f} vs ORB high {orb.orb_high:.2f}): {'✅ PASS' if prev_candle_ok else '❌ FAIL'}")
                else:
                ...
                # Rev 00261: Additional diagnostic if validation fails
        ...
        try:
            ...
            if distance_from_low <= -self.so_inverse_threshold:
                ...
                if validation_close_715 is not None:
                    ...
                    log.info(f"   Previous candle check (7:15 close {validation_close_715:.2f} vs ORB low {orb.orb_low:.2f}): {'✅ PASS' if prev_candle_ok else '❌ FAIL'}")
                else:
