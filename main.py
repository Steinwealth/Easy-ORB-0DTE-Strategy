...
async def start_http_server():
    ...
    try:
        ...
        async def handle_validation_candle_700(request):
            ...
            try:
                logger = logging.getLogger("improved_main")
                logger.info("PIPELINE | STEP 2 TRIGGER | validation-candle-700 endpoint called | 7:00 AM PT")
        ...
        async def handle_prefetch_validation_715(request):
            ...
            try:
                logger = logging.getLogger("improved_main")
                logger.info("PIPELINE | STEP 3 TRIGGER | prefetch-validation-715 endpoint called | 7:15 AM PT")
                system = get_integrated_system()
        ...
        async def handle_end_of_day_report(request):
            ...
            - ORB EOD: This endpoint (removed from internal trading loop)
            - 0DTE EOD: This endpoint (removed from internal trading loop)
        ...
        async def handle_cleanup_images(request):
            ...
            try:
                ...
                # Run image cleanup if requested
                if cleanup_images:
                    ...
                    if os.path.exists(cleanup_images_script):
                        ...
                        logger.warning(f"⚠️ Image cleanup script not found: {cleanup_images_script}")
                        results["results"]["images"] = {
                            "success": False,
                            "error": f"Script not found: {cleanup_images_script}"
                        }
                
                # Run revision cleanup if requested
                if cleanup_revisions:
                    logger.info("📦 Running revision cleanup script...")
                    ...
                    if os.path.exists(cleanup_revisions_script):
                        try:
                            ...
                            logger.info(f"✅ Revision cleanup completed (return code: {result.returncode})")
                        except subprocess.TimeoutExpired:
                            logger.error("❌ Revision cleanup timed out after 10 minutes")
                            results["results"]["revisions"] = {
                                "success": False,
                                "error": "Timeout after 10 minutes"
                            }
                        ...
                        logger.warning(f"⚠️ Revision cleanup script not found: {cleanup_revisions_script}")
                        results["results"]["revisions"] = {
                            "success": False,
                            "error": f"Script not found: {cleanup_revisions_script}"
                        }
                ...
                ) if results["results"] else False
                
                if not all_success:
                    results["status"] = "partial_success" if results["results"] else "error"
                
                return web.json_response(results, status=200 if all_success else 500)
    ...
    try:
        ...
        if ENABLE_0DTE_STRATEGY:
            ...
            try:
                ...
                if etrade_options_api and not is_demo_mode:
