...
**Version**: Rev 00280 (aligned with Strategy; validation candle explicit 7:15 close + GCS persist/load)  
...
|---|------|--------|----------------|
| 1 | **Signal collection** | ORB + 7:00–7:15 validation candle (broker-only). Explicit 7:15 close passed into rules when prefetched (Rev 00279); validation candle persisted to GCS and loaded if scan runs on another instance. | SO signals only; ORR separate. See [SignalRulesChecklist.md](SignalRulesChecklist.md). |
| 2 | **Priority ranking (ORB SO only)** | `calculate_so_priority_score` → sort by score | VWAP 27%, RS vs SPY 25%, ORB vol 22%, confidence 13%, RSI 10%, ORB range 3%. 0DTE uses a different formula (no RS vs SPY — see Strategy.md). |
| 3 | **Adaptive selection** | Expense-ratio filter; protected top 3 | Targets 15→12→10→8 (expense ratio ≤30%); fallback top 8 affordable. |
| 4 | **Assign `priority_rank`** | Before batch sizing (Rev 00099) | Rank 1..N so Risk Manager can apply correct multipliers. |
| 5 | **Batch position sizing** | `calculate_batch_position_sizes()` (Risk Manager) | **6 steps:** (1) Rank multipliers, (2) Max position cap 35%, (3) ADV cap 1% (Slip Guard), (4) Normalize to SO capital, (5) Whole-share rounding, (6) Post-rounding redistribution. |
| 6 | **Filter quantity = 0** | After batch sizing (Rev 00105) | Signals with 0 shares are dropped; only executable signals proceed. |
| 7 | **Execute** | `_batch_execute_live_signals` → E*TRADE | Orders sorted by `priority_rank` (best first); concurrency 3, 1.2s spacing. |
...
### **Latest Updates (February 2026 - Rev 00279/00280)** ⭐
...
*Last Updated: February 2026*  
*Version: Rev 00280 (Validation candle fix 00279; STEP 4 data-source diagnostic)*  
