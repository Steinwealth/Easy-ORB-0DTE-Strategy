...
**Version**: Rev 00260 (aligned with Easy ORB 0DTE Strategy; pipeline logging Rev 00260; Feb19: validation candle 7:00/7:15 scheduler jobs)  
...
- **Broker Configuration**: Multi-broker support via `configs/broker-config.env` (Rev 00245)
...
- **Configuration Files**: Loads `broker-config.env` (Rev 00245) for broker and account selection
...
- **Configuration File**: `configs/broker-config.env`
...
- **URL**: https://YOUR_OAUTH_WEB_APP_URL
...
│  │  Revision: YOUR_PROJECT_ID-00200-nqd            │  │
...
│  │                                                       │  │
│  │  Buckets:                                            │  │
│  │  ├── YOUR_PROJECT_ID-trades/                    │  │
│  │  │   └── Trade history                               │  │
│  │  ├── YOUR_PROJECT_ID-state/                     │  │
│  │  │   └── System state                                │  │
│  │  └── YOUR_PROJECT_ID-logs/                      │  │
│  │      └── Performance logs                            │  │
│  └─────────────────────────────────────────────────────┘  │
│                                                             │
│  ┌─────────────────────────────────────────────────────┐  │
...
- **Region**: us-central1
- **Project**: YOUR_PROJECT_ID (ID: YOUR_PROJECT_NUMBER)
...
- `broker-config.env`: Broker and account selection (Rev 00245) ⭐ **PRIMARY**
...
**Key Settings**:
- `BROKER_TYPE`: etrade|ib|robinhood (Primary broker - in `broker-config.env`)
- `ETRADE_ORB_ACCOUNT_ID`: 775690861 (ORB Strategy account)
- `ETRADE_0DTE_ACCOUNT_ID`: 215107721 (0DTE Strategy account)
...
- **Renewal**: Daily via web app (https://YOUR_OAUTH_WEB_APP_URL)
...
*Last Updated: February 19, 2026*  
*Version: Rev 00260 (pipeline logging, validation candle 7:00/7:15 jobs); Rev 00246 (0DTE Priority Formula v1.1, Direction-Aware Red Day, Expanded Delta Selection, Comprehensive Logging)*  
