...
class ConvexEligibilityFilter:
    ...
    def calculate_eligibility_score(
        ...
        """
        # Normalize so orb_high/orb_low/current_price/orb_range_pct available from orb_data/price when missing
        signal = dict(signal)
        ...
        if isinstance(orb_data, dict):
            ...
            if (signal.get('orb_range_pct') is None or signal.get('orb_range_pct') == 0) and orb_data.get('orb_range_pct') is not None:
                signal['orb_range_pct'] = orb_data['orb_range_pct']
        ...
        # 3. Red Day Status (15% weight) - Rev 00246: Direction-aware
        # LONG (CALL): Red Day prevents trading (declining market not good for calls)
        # SHORT (PUT): Red Day is GOOD (declining market is perfect for puts)
        is_red_day = signal.get('is_red_day', False)
        direction = signal.get('side', 'LONG').upper()
        
        if direction == 'SHORT':
            # SHORT (PUT) trades: Red Day is GOOD (allow, even encourage)
            if is_red_day:
                score += 0.20  # Bonus: Red Day is perfect for PUT trades
            else:
                score += 0.10  # Partial score: Not Red Day is OK for PUTs
        else:
            # LONG (CALL) trades: Red Day prevents trading (must NOT be red day)
            if not is_red_day:
                score += 0.15  # Not Red Day is required for CALL trades
        max_score += 0.15
    ...
    def is_eligible(
        ...
        # Normalize signal so Convex checks see orb_high, orb_low, current_price (from orb_data/price when missing)
        signal = dict(signal)
        orb_data = signal.get('orb_data') or {}
        if isinstance(orb_data, dict):
            if signal.get('orb_high') is None and orb_data.get('orb_high') is not None:
        ...
        # 3. Red Day Check - Rev 00246: Direction-aware filtering
        # LONG (CALL): Red Day prevents trading (declining market not good for calls)
        # SHORT (PUT): Red Day is GOOD (declining market is perfect for puts)
        is_red_day = signal.get('is_red_day', False)
        direction = signal.get('side', 'LONG').upper()
        ...
        if direction == 'SHORT':
            # SHORT (PUT) trades: Red Day is GOOD
            if is_red_day:
                checks['not_red_day'] = True  # Check passes (using same key for consistency)
                eligibility_reasons.append("✅ Red Day detected - PERFECT for SHORT (PUT) trades (declining market)")
            else:
                checks['not_red_day'] = True  # Check passes (not Red Day is also OK for PUTs)
                eligibility_reasons.append("NOT a Red Day (OK for SHORT PUT trades)")
        else:
            # LONG (CALL) trades: Red Day prevents trading
            if not is_red_day:
                checks['not_red_day'] = True
                eligibility_reasons.append("NOT a Red Day (required for LONG CALL trades)")
            else:
                rejection_reasons.append("❌ Red Day detected - REJECTED for LONG (CALL) trades (declining market not good for calls)")
    ...
    def filter_signals(
        ...
        log.info(f"   Filter criteria: 8 checks (Volatility 40%, Range/ATR 25%, Red Day 15%, Break 10%, Volume 10%, VWAP 10%, Momentum 10%, Regime 10%)")
        ...
        for signal in signals:
            ...
            # Rev 00246: Log per-signal filtering result (for debugging)
            # Enhanced logging for Red Day and Hard Gate adjustments
            is_red_day = signal.get('is_red_day', False)
            red_day_status = "Red Day" if is_red_day else "Not Red Day"
            
            if is_eligible:
                log.debug(f"   ✅ {symbol} {direction}: ELIGIBLE (Score: {score:.3f}, {red_day_status}, Passed: {len(eligibility_reasons)} checks)")
                # Log Red Day decision for eligible signals
                if 'Red Day' in ' '.join(eligibility_reasons):
                    log.debug(f"      Red Day decision: {'✅ ALLOWED (SHORT PUT on Red Day)' if direction == 'SHORT' and is_red_day else '✅ NOT Red Day (required for LONG CALL)'}")
            else:
                log.debug(f"   ❌ {symbol} {direction}: REJECTED (Score: {score:.3f}, {red_day_status}, Failed: {len(rejection_reasons)} checks)")
                if len(rejection_reasons) > 0:
                    log.debug(f"      Top rejection: {rejection_reasons[0]}")
                # Log Red Day decision for rejected signals
                red_day_rejections = [r for r in rejection_reasons if 'Red Day' in r]
                if red_day_rejections:
                    log.debug(f"      Red Day decision: {red_day_rejections[0]}")
