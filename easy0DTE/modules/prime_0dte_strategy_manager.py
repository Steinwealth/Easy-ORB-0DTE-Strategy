...
class Prime0DTEStrategyManager:
    ...
    async def listen_to_orb_signals(
        ...
        log.info(f"📥 0DTE Strategy: Received {len(orb_signals)} ORB signals for processing")
        ...
        orb_symbols = [s.get('symbol', 'UNKNOWN') for s in orb_signals]
        dte_targets_in_signals = [s for s in orb_symbols if s in self.target_symbols]
        log.info(f"   ORB signals matching 0DTE targets: {len(dte_targets_in_signals)}/{len(orb_signals)}")
        ...
        log.info(f"🔍 Applying Convex Eligibility Filter (min score: 0.75)...")
        ...
        if not eligible_results:
            log.warning(f"⚠️ 0DTE Strategy: No signals passed Convex Eligibility Filter (0/{len(orb_signals)})")
            log.warning(f"   All {len(orb_signals)} ORB signals were filtered out by Convex Eligibility Filter")
            return []
        
        log.info(f"✅ {len(eligible_results)}/{len(orb_signals)} signals passed Convex Eligibility Filter")
        ...
        for result in long_results:
            ...
            # Rev 00246 (Jan 19, 2026): Expanded delta range from 0.15-0.30 to 0.15-0.35
            # SPX: Index options (professional/institutional standard)
            if target_symbol == 'SPX':
                if orb_range_pct >= 0.50:
                    target_delta = 0.30  # High volatility: 30 delta (↑ from 0.25)
                    spread_width = 10.0  # $10 spread for SPX index options
                elif orb_range_pct >= 0.35:
                    target_delta = 0.25  # Moderate volatility: 25 delta (↑ from 0.20)
            ...
            # Rev 00246: Default for other 0DTE symbols (NVDA, AMD, TSLA, META, AMZN, AAPL, MSFT, IWM)
            # Use 15-35 delta range for cheap gamma explosion (expanded from 15-30)
            else:
                if orb_range_pct >= 0.50:
                    target_delta = 0.35  # High volatility: 35 delta (NEW - higher for high volatility)
        ...
        for result in short_results:
            ...
            # Rev 00246 (Jan 19, 2026): Expanded delta range from 0.15-0.30 to 0.15-0.35 for PUT options
            # Premium target: $0.20-$0.60, 1-3 strikes OTM
            if target_symbol == 'SPX':
                if orb_range_pct >= 0.50:
                    target_delta = 0.30  # High volatility: 30 delta (↑ from 0.25)
                ...
                # Rev 00246: Default for other 0DTE symbols (expanded to 0.35 delta)
                if orb_range_pct >= 0.50:
                    target_delta = 0.35  # High volatility: 35 delta (NEW - higher for high volatility)
                    spread_width = 2.0
                elif orb_range_pct >= 0.35:
    ...
    def _calculate_priority_score(self, signal: DTE0Signal) -> float:
        ...
        try:
            ...
            # Calculate final priority score (Formula v1.1 - Rev 00246)
            # ALL 0DTE symbols use the same formula (no RS vs SPY for any symbol)
            priority_score = (
                breakout_score * 0.35 +      # 35% - ORB Breakout % (↑ from 30%)
                range_score * 0.30 +         # 30% - ORB Range % (↑ from 25%)
                volume_score * 0.20 +        # 20% - Volume (same)
                eligibility_normalized * 0.15    # 15% - Eligibility Score (same)
                # RS vs SPY: REMOVED (not relevant for 0DTE options)
                # Momentum: REMOVED (redundant with breakout score)
            )
    ...
    def _select_strategy_type(
        ...
        - momentum_score ≥ 80, volatility_expanding → 'long_call' or 'long_put' (single-leg, high convexity)
        - momentum_score ≥ 70, quick_expansion → 'momentum_scalper' (ATM debit spread, fast payoff)
        - momentum_score ∈ [45, 70], stable_conditions → 'itm_probability_spread' (ITM debit spread, higher probability)
        - momentum_score ∈ [55, 80] OR default → 'debit_spread' (standard OTM debit spread)
        - momentum_score ≥ 90, trend_day_confirmed → 'lotto' (OTM single-leg, maximum leverage)
        - momentum_score < 45 OR chop_detected → 'no_trade' (skip trade)
        ...
        try:
            ...
            # Check for chop conditions (low momentum, low volume, low confidence)
            is_chop = (
                momentum_score < 45 or
                volume_ratio < 1.2 or
                confidence < 0.60 or
                eligibility_score < 0.75
            )
            
            # CASE E — NO TRADE (Low momentum or chop detected)
            if is_chop:
                log.debug(f"🚫 NO TRADE: Momentum {momentum_score:.1f} < 45 or chop detected (volume: {volume_ratio:.2f}, confidence: {confidence:.2f})")
                return 'no_trade'
            
            # CASE D — LOTTO MODE (RARE, OPTIONAL)
            # momentum_score ≥ 90, trend_day_confirmed, high volatility
            if momentum_score >= 90.0 and orb_range_pct >= 0.50 and self.enable_lotto_sleeve:
                log.debug(f"🎰 LOTTO MODE: Momentum {momentum_score:.1f} ≥ 90, high volatility ({orb_range_pct:.2f}%)")
                return 'lotto'
            
            # CASE A — LONG OPTION (Momentum Expansion)
            # momentum_score ≥ 80, volatility_expanding
            if momentum_score >= 80.0 and orb_range_pct >= 0.40:
                if direction == 'LONG':
                    log.debug(f"🚀 LONG CALL: Momentum {momentum_score:.1f} ≥ 80, volatility expanding ({orb_range_pct:.2f}%)")
                    return 'long_call'
                else:
                    log.debug(f"🚀 LONG PUT: Momentum {momentum_score:.1f} ≥ 80, volatility expanding ({orb_range_pct:.2f}%)")
                    return 'long_put'
            
            # CASE 2 — MOMENTUM SCALPER (Quick Expansion)
            # momentum_score ≥ 70, quick_expansion_expected
            if momentum_score >= 70.0 and orb_range_pct >= 0.35:
                log.debug(f"🎯 MOMENTUM SCALPER: Momentum {momentum_score:.1f} ≥ 70, quick expansion expected")
                return 'momentum_scalper'
            
            # CASE C — ITM PROBABILITY SPREAD (Easy Mode)
            # momentum_score ∈ [45, 70], structure clean
            if 45.0 <= momentum_score < 70.0 and orb_range_pct >= 0.25:
                log.debug(f"📊 ITM PROBABILITY SPREAD: Momentum {momentum_score:.1f} ∈ [45, 70], stable conditions")
                return 'itm_probability_spread'
            
            # CASE B — DEBIT SPREAD (Default / Core Trade) ⭐ MOST COMMON
            # momentum_score ∈ [55, 80] OR default fallback
            if momentum_score >= 55.0:
                log.debug(f"🎯 DEBIT SPREAD: Momentum {momentum_score:.1f} ∈ [55, 80] (default strategy)")
                return 'debit_spread'
            
            # Fallback to debit spread for any remaining cases
            log.debug(f"🎯 DEBIT SPREAD: Default fallback (momentum: {momentum_score:.1f})")
            return 'debit_spread'
            
        except Exception as e:
            log.error(f"Error selecting strategy type: {e}")
            # Default to debit spread on error
            return 'debit_spread'
