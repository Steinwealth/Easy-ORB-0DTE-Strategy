...
class OptionsChainManager:
    ...
    def validate_liquidity(self, contract: OptionContract) -> Tuple[bool, List[str]]:
        ...
        if contract.open_interest < self.min_open_interest:
            ...
            log.debug(f"   ❌ Hard Gate: OI {contract.open_interest} < {self.min_open_interest} for {contract.symbol} {contract.option_type} ${contract.strike}")
        else:
            log.debug(f"   ✅ Hard Gate: OI {contract.open_interest} ≥ {self.min_open_interest} for {contract.symbol} {contract.option_type} ${contract.strike}")
        ...
        if contract.bid_ask_spread_pct > self.max_bid_ask_spread_pct:
            ...
            log.debug(f"   ❌ Hard Gate: Spread {contract.bid_ask_spread_pct:.2f}% > {self.max_bid_ask_spread_pct:.1f}% for {contract.symbol} {contract.option_type} ${contract.strike} (Bid: ${contract.bid:.2f}, Ask: ${contract.ask:.2f}, Mid: ${contract.mid_price:.2f})")
        else:
            log.debug(f"   ✅ Hard Gate: Spread {contract.bid_ask_spread_pct:.2f}% ≤ {self.max_bid_ask_spread_pct:.1f}% for {contract.symbol} {contract.option_type} ${contract.strike}")
        ...
        if contract.volume < self.min_volume:
            ...
            log.debug(f"   ❌ Hard Gate: Volume {contract.volume} < {self.min_volume} for {contract.symbol} {contract.option_type} ${contract.strike}")
        else:
            log.debug(f"   ✅ Hard Gate: Volume {contract.volume} ≥ {self.min_volume} for {contract.symbol} {contract.option_type} ${contract.strike}")
        ...
        if is_valid:
            ...
            log.warning(f"   ❌ Hard Gate FAILED: {contract.symbol} {contract.option_type} ${contract.strike} - {', '.join(reasons)}")
