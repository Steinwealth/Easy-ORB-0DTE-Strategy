...
class ETradeOptionsAPI:
    ...
    async def get_option_quote(
        ...
        """
        try:
            # Fetch options chain for the symbol and expiry
            chain = await self.fetch_options_chain(
                symbol=symbol,
                expiry=expiry,
                strike_count=20,  # Fetch enough strikes to find our target
                include_greeks=True
            )
            ...
            # Find contract with matching strike (allow small tolerance for float comparison)
            target_contract = None
            for contract in contracts:
                if abs(contract.strike - strike) < 0.01:  # Tolerance for float comparison
                    target_contract = contract
                    break
            
            if target_contract:
                return {
                    'bid': target_contract.bid,
                    'ask': target_contract.ask,
                    'last': target_contract.last,
                    'mid_price': target_contract.mid_price,
                    'volume': target_contract.volume,
                    'open_interest': target_contract.open_interest,
                    'delta': target_contract.delta,
                    'gamma': target_contract.gamma,
                    'theta': target_contract.theta,
                    'vega': target_contract.vega,
                    'implied_volatility': target_contract.implied_volatility
                }
            else:
                log.warning(f"Option contract not found: {symbol} {strike} {option_type} {expiry}")
                return None
                
        except Exception as e:
            log.error(f"Failed to fetch option quote for {symbol} {strike} {option_type}: {e}")
            return None
