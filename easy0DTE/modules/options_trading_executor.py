...
class OptionsTradingExecutor:
    ...
    async def update_positions_with_real_prices(self) -> None:
        """
        ...
        for position in open_positions:
            try:
                ...
                # For demo mode, skip real API calls (use mock pricing)
                if self.demo_mode:
                    # In demo mode, we can estimate from underlying price if available
                    # But for now, keep existing behavior (will be handled by mock executor)
                    continue
                
                # Live mode: Fetch real options prices from E*TRADE
                if not self.demo_mode and self.etrade_options_api:
                    # Get expiry and option type from position structure
                    expiry = None
                    option_type = None
                    strike = None
                    long_strike = None
                    ...
                    if position.position_type == 'debit_spread' and position.debit_spread:
                        expiry = position.debit_spread.expiry
                        option_type = position.debit_spread.option_type.upper()
                        long_strike = position.debit_spread.long_strike
                        short_strike = position.debit_spread.short_strike
                    elif position.position_type == 'credit_spread' and position.credit_spread:
                        expiry = position.credit_spread.expiry
                        option_type = position.credit_spread.option_type.upper()
                        long_strike = position.credit_spread.long_strike
                        short_strike = position.credit_spread.short_strike
                    elif position.position_type == 'lotto' and position.lotto_contract:
                        expiry = position.lotto_contract.expiry
                        option_type = position.lotto_contract.option_type.upper()
                        strike = position.lotto_contract.strike
                    
                    if not expiry or not option_type:
                        log.warning(f"   Position {position.position_id} missing expiry or option_type, skipping")
                        continue
                    
                    # Convert expiry format if needed (YYYY-MM-DD -> YYYYMMDD)
                    expiry_formatted = expiry.replace('-', '') if '-' in expiry else expiry
                    
                    if position.position_type == 'lotto' and strike:
                        # Single-leg position: Fetch quote for the strike
                        quote = await self.etrade_options_api.get_option_quote(
                        ...
                        if quote:
                            # Use mid_price for fair value, or last if mid_price not available
                            current_value = quote.get('mid_price') or quote.get('last', 0.0)
                            log.debug(f"   {position.symbol} {strike} {option_type}: ${current_value:.2f} (bid: ${quote.get('bid', 0):.2f}, ask: ${quote.get('ask', 0):.2f})")
                    
                    elif position.position_type == 'debit_spread' and long_strike and short_strike:
                        # Debit spread: Fetch quotes for both legs
                        long_quote = await self.etrade_options_api.get_option_quote(
                            symbol=position.symbol,
                            strike=long_strike,
                            expiry=expiry_formatted,
                            option_type=option_type
                        ...
                        short_quote = await self.etrade_options_api.get_option_quote(
                            symbol=position.symbol,
                            strike=short_strike,
                            expiry=expiry_formatted,
                            option_type=option_type
                        ...
                        if long_quote and short_quote:
                            # Spread value = long leg - short leg
                            long_value = long_quote.get('mid_price') or long_quote.get('last', 0.0)
                            short_value = short_quote.get('mid_price') or short_quote.get('last', 0.0)
                            current_value = long_value - short_value
                            log.debug(f"   {position.symbol} {long_strike}/{short_strike} {option_type} debit spread: ${current_value:.2f} (long: ${long_value:.2f}, short: ${short_value:.2f})")
                    
                    elif position.position_type == 'credit_spread' and long_strike and short_strike:
                        # Credit spread: Fetch quotes for both legs
                        short_quote = await self.etrade_options_api.get_option_quote(
                            symbol=position.symbol,
                            strike=short_strike,  # Credit spread: short leg is higher strike
                            expiry=expiry_formatted,
                            option_type=option_type
                        )
                        ...
                        long_quote = await self.etrade_options_api.get_option_quote(
                            symbol=position.symbol,
                            strike=long_strike,  # Credit spread: long leg is lower strike
                            expiry=expiry_formatted,
                            option_type=option_type
                        )
                        
                        if long_quote and short_quote:
                            # Spread value = credit received - current cost to close
                            # Current cost to close = short leg - long leg
                            short_value = short_quote.get('mid_price') or short_quote.get('last', 0.0)
                            long_value = long_quote.get('mid_price') or long_quote.get('last', 0.0)
                            cost_to_close = short_value - long_value
                            # For credit spreads, profit = credit received - cost to close
                            # But current_value should represent current spread value
                            current_value = cost_to_close
                            log.debug(f"   {position.symbol} {short_strike}/{long_strike} {option_type} credit spread: cost to close ${current_value:.2f}")
                    
                    # Update position value if we got a quote
                    if current_value is not None and current_value > 0:
                        await self.update_position_value(position.position_id, current_value)
                    else:
                        log.warning(f"   Could not fetch quote for position {position.position_id}, keeping existing value")
